...
presets.district = new ErrorHandler({
	...
	fail: {
		body: [{
			...
			rejectComment: 'District not found.'
		}],
		status: [{
			status: 404,
			rejectComment: 'District not found.'
...
presets.login = new ErrorHandler({
	...
	fail: {
		body: [{
				text: 'password-error',
				rejectComment: 'Username/Password is incorrect.'
			},
			{
			...
			{
				text: 'No Campus Application selected',
				rejectComment: 'District/School name failed.'
			}
		],
		status: [{
			status: 400,
			rejectComment: 'bad request'
		}]
	},
	success: {
		body: ['success'],
...
presets.notificationToggle = new ErrorHandler({
	...
	fail: {
		body: [{
				text: 'Cannot mark other user\'s notification as read or unread',
				rejectComment: 'Notification ID not found during read toggle.'
			},
			{
				text: 'invalid input',
				rejectComment: 'Invalid input.'
			},
			{
				text: 'No Campus Application selected',
				rejectComment: 'District/School name failed.'
